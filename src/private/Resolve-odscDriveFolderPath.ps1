function Resolve-odscDriveFolderPath {
    ...
    if ([string]::IsNullOrWhiteSpace($RelativePath)) {
        return Invoke-odscApiRequest -Resource "users/${User}/drive/root" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
    }

    $Segments = $RelativePath -split '[\\/]+' | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
    $CurrentResource = "users/${User}/drive/root"
    $CurrentItem = Invoke-odscApiRequest -Resource $CurrentResource -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
    ...
    foreach ($Segment in $Segments) {
        ...
        if (-not $Child) {
            ...
            if ($PSCmdlet.ShouldProcess("${User}'s OneDrive", "Creating folder '$Segment' in '$RelativePath'")) {
                $Child = Invoke-odscApiRequest @CreateRequest
            }
        }
