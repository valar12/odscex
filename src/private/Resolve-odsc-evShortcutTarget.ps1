function Resolve-odsc-evShortcutTarget {
    [CmdletBinding()]
    param(
    ...
    $SiteResponse = Invoke-odsc-evApiRequest -Resource "sites/${SiteDomain}:${SiteResource}" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
    if (!($SiteResponse)) {
    ...
    if ($DocumentLibraryId) {
        $DocumentLibraryResponse = Invoke-odsc-evApiRequest -Resource "sites/${SiteIdRaw}/lists/${DocumentLibraryId}" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
    } else {
        $EscapedLibrary = $DocumentLibrary.Replace("'", "''")
        $LibraryMatches = Invoke-odsc-evApiRequest -Resource "sites/${SiteIdRaw}/lists?`$filter=displayName eq '${EscapedLibrary}'" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) -AllPages

        if ((!$LibraryMatches) -or ($LibraryMatches.Count -eq 0)) {
            $LibraryMatches = Invoke-odsc-evApiRequest -Resource "sites/${SiteIdRaw}/lists?`$filter=startsWith(displayName,'${EscapedLibrary}')" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) -AllPages
        }
    ...
    if ($FolderPath) {
        $ListDrive = Invoke-odsc-evApiRequest -Resource "sites/${SiteIdRaw}/lists/${ResolvedLibraryId}/drive" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
        $EncodedFolderPath = ConvertTo-odsc-evGraphDrivePath -Path $FolderPath
        $DriveItem = Invoke-odsc-evApiRequest -Resource "drives/$($ListDrive.id)/root:/${EncodedFolderPath}" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) -DoNotUsePrefer
