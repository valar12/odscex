function Remove-odsc-ev {
    ...
    process {
        ...
        $ShortcutResponse = Invoke-odsc-evApiRequest -Resource $ShortcutResource -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
        ...
        if ($PSCmdlet.ShouldProcess("${User}'s OneDrive", "Removing shortcut '$($ShortcutName)'")) {
            $RemoveResponse = Invoke-odsc-evApiRequest -Resource $ShortcutResource -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Delete)

            if ($PassThru) {
                return Write-odsc-evResult -User $User -ShortcutName $ShortcutName -Action 'Remove' -Status 'Removed' -Response $ShortcutResponse
            }
