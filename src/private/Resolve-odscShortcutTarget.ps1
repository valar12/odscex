function Resolve-odscShortcutTarget {
    [CmdletBinding()]
    param(
    ...
    $SiteResponse = Invoke-odscApiRequest -Resource "sites/${SiteDomain}:${SiteResource}" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
    if (!($SiteResponse)) {
    ...
    if ($DocumentLibraryId) {
        $DocumentLibraryResponse = Invoke-odscApiRequest -Resource "sites/${SiteIdRaw}/lists/${DocumentLibraryId}" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
    } else {
        $EscapedLibrary = $DocumentLibrary.Replace("'", "''")
        $LibraryMatches = Invoke-odscApiRequest -Resource "sites/${SiteIdRaw}/lists?`$filter=displayName eq '${EscapedLibrary}'" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) -AllPages

        if ((!$LibraryMatches) -or ($LibraryMatches.Count -eq 0)) {
            $LibraryMatches = Invoke-odscApiRequest -Resource "sites/${SiteIdRaw}/lists?`$filter=startsWith(displayName,'${EscapedLibrary}')" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) -AllPages
        }
    ...
    if ($FolderPath) {
        $ListDrive = Invoke-odscApiRequest -Resource "sites/${SiteIdRaw}/lists/${ResolvedLibraryId}/drive" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get)
        $EncodedFolderPath = ConvertTo-odscGraphDrivePath -Path $FolderPath
        $DriveItem = Invoke-odscApiRequest -Resource "drives/$($ListDrive.id)/root:/${EncodedFolderPath}" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) -DoNotUsePrefer
