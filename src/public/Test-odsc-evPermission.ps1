function Test-odsc-evPermission {
    [CmdletBinding()]
    param(
    ...
    try {
        Invoke-odsc-evApiRequest -Resource 'organization' -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) | Out-Null
    ...
    if ($UserPrincipalName -or $UserObjectId) {
        ...
        try {
            Invoke-odsc-evApiRequest -Resource "users/${User}/drive" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) | Out-Null
    ...
    if ($Uri) {
        try {
            $SiteUri = [uri]$Uri
            Invoke-odsc-evApiRequest -Resource "sites/$($SiteUri.Authority):$($SiteUri.AbsolutePath.TrimEnd('/'))" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) | Out-Null
