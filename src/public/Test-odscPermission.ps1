function Test-odscPermission {
    ...
    try {
        Invoke-odscApiRequest -Resource 'organization' -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) | Out-Null
    ...
    if ($UserPrincipalName -or $UserObjectId) {
        ...
        try {
            Invoke-odscApiRequest -Resource "users/${User}/drive" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) | Out-Null
    ...
    if ($Uri) {
        try {
            $SiteUri = [uri]$Uri
            Invoke-odscApiRequest -Resource "sites/$($SiteUri.Authority):$($SiteUri.AbsolutePath.TrimEnd('/'))" -Method ([Microsoft.PowerShell.Commands.WebRequestMethod]::Get) | Out-Null
